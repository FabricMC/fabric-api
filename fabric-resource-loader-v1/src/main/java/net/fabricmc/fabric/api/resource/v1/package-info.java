...
/// Mods can "bundle" resource packs with the mod by putting files inside the `assets` or `data`
/// sub-directories of the `resources` directory. They are always enabled, initially loaded after the vanilla pack,
...
/// `resources/resourcepacks/` directory. For example, a built-in data pack with the ID `example:test` should be placed
/// under `resources/resourcepacks/test/data/`. The packs are then registered with
/// [net.fabricmc.fabric.api.resource.v1.ResourceLoader#registerBuiltinPack(net.minecraft.resources.Identifier, net.fabricmc.loader.api.ModContainer, net.fabricmc.fabric.api.resource.v1.pack.PackActivationType)].
...
/// under `resources/programmer_art/assets/` or `resources/high_contrast/assets/`, respectively.
...
/// #### Example
/// Mod A (`mod_a` provides a bundled resource pack with both Programmer Art and High Contrast support.
/// Mod B (`mod_b`) provides a bundled resource pack and one built-in resource pack, Extra (`mod_b:extra`).
/// When neither the Programmer Art nor High Contrast is enabled, the user sees "Vanilla", and "Extra" in the
/// Resource Packs screen. Internally, between Vanilla and Extra packs, two hidden packs exist: `mod_a` and `mod_b`.
///
/// Suppose the user then enables both the Programmer Art and High Contrast, and the Resource Packs screen lists
/// "Vanilla", Programmer Art, "Extra", and High Contrast. Internally, there are 4 hidden packs:
///
...
/// [net.fabricmc.fabric.api.resource.v1.ResourceLoader#registerReloadListener(net.minecraft.resources.Identifier, net.minecraft.server.packs.resources.PreparableReloadListener)],
/// which are triggered when resources are reloaded.
/// A reload listener can be ordered to depend on other reload listeners using [net.fabricmc.fabric.api.resource.v1.ResourceLoader#addListenerOrdering(net.minecraft.resources.Identifier, net.minecraft.resources.Identifier)],
/// keys for Vanilla reload listeners can be found at [net.fabricmc.fabric.api.resource.v1.reloader.ResourceReloaderKeys].
@NullMarked
