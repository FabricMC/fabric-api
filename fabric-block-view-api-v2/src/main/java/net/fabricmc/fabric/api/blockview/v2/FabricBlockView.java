...
///
/// Note: This interface is automatically implemented on all [BlockGetter] instances via Mixin and interface injection.
public interface FabricBlockView {
	/// Retrieves block entity render data for a given block position.
	///
	/// This method must be used instead of [BlockGetter#getBlockEntity(BlockPos)] in cases
	/// where the user knows that the current context may be multithreaded, such as chunk building, to
	/// ensure thread safety and data consistency. Using a [BlockEntity] directly may not be
	/// thread-safe since it may lead to non-atomic modification of the internal state of the
	/// [BlockEntity] (such as through lazy computation). Using a [BlockEntity] directly
	/// may not be consistent since the internal state of the [BlockEntity] may change on a
	/// different thread.
	///
	...
	/// not limited to, `FabricBakedModel#emitBlockQuads` (block models),
	/// `BlockColor#getColor` (block color), and
	/// `FabricBlock#getAppearance` (block appearance computation).
	///
	...
	/// to implement these checks is to use `instanceof`, since it always returns `false`
	/// if the object is null. If the `instanceof` returns `false`, a fallback path should
	/// be used.
	///
	...
	/// multiple calls of this method. See [#getBiomeFabric(BlockPos)] for more information.
	///
	...
	/// [#hasBiomes()] returns `true`, this method will always return a non-null
