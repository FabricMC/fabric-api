...
public interface HudElementRegistry {
	 ...
	 *
	 * <p>Use event phases to order hud elements within an event.
	 *
	 ...
	 * @return an event to add elements to
	 */
	static Event<HudElement> before(Identifier beforeThis) {
		Preconditions.checkNotNull(beforeThis, "beforeThis");
		return HudElementRegistryImpl.before(beforeThis);
	}
	 ...
	 * @return an event to add elements to
	 */
	static Event<HudElement> after(Identifier afterThis) {
		Preconditions.checkNotNull(afterThis, "afterThis");
		return HudElementRegistryImpl.after(afterThis);
	}
}
