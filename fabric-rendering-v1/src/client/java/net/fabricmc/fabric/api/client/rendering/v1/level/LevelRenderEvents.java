...
///
/// These events can be separated into two categories, the "extraction" events and the "drawing" events,
...
///
/// To attach modded data to vanilla render states, see [FabricRenderState][net.fabricmc.fabric.api.client.rendering.v1.FabricRenderState].
/// Only attach the minimum data needed for rendering. Do not attach objects that are not thread-safe such as [net.minecraft.client.multiplayer.ClientLevel].
public final class LevelRenderEvents {
	...
	///
	/// Use this to extract custom data needed when decorating or replacing
	...
	/// Only attach the minimum data needed for rendering. Do not attach objects that are not thread-safe such as [net.minecraft.client.multiplayer.ClientLevel].
	///
	/// Setting the outline render state to null by any event subscriber
	/// will cancel the default block outline render and suppress the [#BEFORE_BLOCK_OUTLINE] event.
	...
	///
	/// This event should NOT be used for general-purpose replacement of
	...
	/// should instead subscribe to [#BEFORE_BLOCK_OUTLINE].
	...
	/// Called after the [SOLID][net.minecraft.client.renderer.chunk.ChunkSectionLayer#SOLID], [CUTOUT][net.minecraft.client.renderer.chunk.ChunkSectionLayer#CUTOUT],
	/// and [CUTOUT_MIPPED][net.minecraft.client.renderer.chunk.ChunkSectionLayer#CUTOUT] terrain layers are drawn to the framebuffer,
	...
	///
	/// Note that 3rd-party renderers may combine these passes or otherwise alter the
	...
	///
	/// Because solid and cutout quads are depth-tested, order of output does not matter except to improve
	...
	/// extract them during [#END_EXTRACTION].
	...
	///
	/// Use to draw on top of the main and entity framebuffer targets
	...
	/// was set to null in [#AFTER_BLOCK_OUTLINE_EXTRACTION].
	///
	/// Use this to replace the default block outline rendering for specific blocks that
	...
	/// renders will appear.
	///
	/// Returning false from any event subscriber will cancel the default block
	...
	///
	/// Use to draw on top of the level before hand and GUI are drawn.
