...
///
/// The renderers are registered with [net.fabricmc.fabric.api.client.rendering.v1.ArmorRenderer#register(Factory, ItemLike...)]
/// or [net.fabricmc.fabric.api.client.rendering.v1.ArmorRenderer#register(ArmorRenderer, ItemLike...)].
@FunctionalInterface
public interface ArmorRenderer {
	...
	/// Helper method for rendering a [TransformCopyingModel], which will copy transforms from a source model to
	...
	/// @param setDelegateAngles     `true` if the [Model#setupAnim(Object)] method should be called for the
	///                                             delegate model after it is called for the source model
	/// @param nodeCollector         the [OrderedSubmitNodeCollector]
	/// @param poseStack             the pose stack
	/// @param renderType            the render type
	/// @param light                 packed lightmap coordinates
	/// @param overlay               packed overlay texture coordinates
	/// @param tintedColor           the color to tint the model with
	/// @param sprite                the sprite to render the model with, or `null` to use the render layer instead
	/// @param outlineColor          the outline color of the model
	/// @param crumblingOverlay      the crumbling overlay, or `null` for no crumbling overlay
	...
	/// Helper method for rendering a [TransformCopyingModel], which will copy transforms from its source model to
	...
	///
	/// @param poseStack                 the pose stack
	/// @param submitNodeCollector       the [SubmitNodeCollector] instance
	...
	/// @param light                     packed lightmap coordinates
	/// @param contextModel              the model provided by [net.minecraft.client.renderer.entity.layers.RenderLayer#getParentModel()]
	...
	///
	/// Note that the item will never be rendered by vanilla code if it has an armor model defined
	/// by the [minecraft:equippable][net.minecraft.core.component.DataComponents#EQUIPPABLE] component.
	...
	/// @return `true` if the head item should be rendered, `false` otherwise
