...
public class TagRemovalInternals {
	...
	private static final ThreadLocal<Map<Identifier, List<TagGroupLoader.TrackedEntry>>> REMOVE_ENTRIES = ThreadLocal.withInitial(HashMap::new);
	...
	public static void addRemoveEntry(Identifier tagId, TagGroupLoader.TrackedEntry entry) {
		...
		TagEntry optional = accessor.fabric_getTag() ? TagEntry.createOptionalTag(accessor.fabric_getId()) : TagEntry.createOptional(accessor.fabric_getId());
		TagGroupLoader.TrackedEntry toAdd = new TagGroupLoader.TrackedEntry(optional, entry.source());
	...
	public static boolean isEntryRemove(TagGroupLoader.TrackedEntry entry) {
	...
	public static void mergeAddedAndRemovedEntries(Identifier tagId, List<TagGroupLoader.TrackedEntry> entries) {
		if (!REMOVE_ENTRIES.get().containsKey(tagId)) {
			return;
		}

		List<TagGroupLoader.TrackedEntry> newEntries = new ArrayList<>();
