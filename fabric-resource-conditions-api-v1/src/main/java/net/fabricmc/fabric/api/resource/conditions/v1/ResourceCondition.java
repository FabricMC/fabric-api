...
/// call [ResourceConditionType#create] and create the type, then register
/// via [ResourceConditions#register].
public interface ResourceCondition {
	/// A codec for a resource condition. It is a map with a `condition` key denoting the ID
	...
	///
	/// @implNote `registryLookup` should never be `null` for supported use cases
	/// (such as recipes or advancements). However, it may be `null` in client-side
	/// resources, or for non-vanilla pack types.
	///
	/// @param registryInfo the registry lookup, or `null` in case registry is unavailable
