...
public final class ResourceConditions {
	...
	/// @throws IllegalArgumentException if `condition` is already registered
	...
	/// @return the condition with ID `id`, or `null` if there is no such condition
	...
	/// A condition that passes if `condition` does not pass. Has ID `fabric:not` and
	/// takes one field, `value`, which is a resource condition.
	public static ResourceCondition not(ResourceCondition condition) {
		return new NotResourceCondition(condition);
	}

	/// A condition that passes if each of the `conditions` passes. Has ID `fabric:and`
	/// and takes one field, `values`, which is a list of resource conditions.
	...
	/// A condition that passes if any of the `conditions` passes. Has ID `fabric:or`
	/// and takes one field, `values`, which is a list of resource conditions.
	...
	/// A condition that passes if each of the specified `modIds` are loaded. Has ID
	/// `all_mods_loaded` and takes one field, `values`, which is a list of strings indicating
	...
	/// A condition that passes if any of the specified `modIds` are loaded. Has ID
	/// `any_mods_loaded` and takes one field, `values`, which is a list of strings indicating
	...
	/// A condition that passes if each of the `tags` exist. This does not check if those tags have
	/// any entries. Has ID `tags_populated` and takes two fields: `registry`, which is the ID
	/// of the registry the tag is for, and `values`, which is a list of the tag IDs to check.
	...
	/// A condition that passes if each of the `features` are enabled. Has ID `features_enabled`
	/// and takes one field, `features`, which is a list of the feature IDs. If there are no IDs to
	...
	/// A condition that passes if each of the `entries` exist. The entries may be from static or
	/// dynamic registries. Has ID `registry_contains` and takes two fields: `registry`, which
	/// is the ID of the registry, and `values`, which is a list of IDs to check.
	/// If there are no IDs to check, it always passes, including in cases where a nonexistent registry is
	/// specified.
	///
	/// @param <T> the type of the tag values
