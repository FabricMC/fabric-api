...
/// to make sure they don't call [#abort], [#commit] or [#close] mistakenly.
///
...
public interface Transaction extends AutoCloseable, TransactionContext {
	...
	///
	/// **Usage of this function is strongly discouraged**, this is why it is deprecated and contains `unsafe` in its name.
	...
	/// the transactions opened with [#openNested] from this transaction.
	///
	...
	/// the **committed** transactions opened with [#openNested] from this transaction.
	/// If this transaction was opened with [#openOuter], all changes are applied.
	/// If this transaction was opened with [#openNested], all changes will be applied when and if the changes of
