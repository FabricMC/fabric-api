...
///
///   - Query which variant is currently in the main slot through [#getItemVariant].
///     **It is important to check this before any operation, to make sure the item variant hasn't changed since the query.**
///   - Query how much of the (non-blank) variant is in the inventory through [#getAmount].
///   - Extract some items from the main slot with [#extract]. In the water bucket example, this can be used for step 1.
///   - Insert some items, either into the main slot if possible or the rest of the inventory otherwise, with [#insert].
///     In the water bucket example, this can be used for steps 2 and 3.
///   - Exchange some of the current variant with another variant through [#exchange].
...
public interface ContainerItemContext {
	...
	/// If the result is not blank, [#getAmount] should be positive.
