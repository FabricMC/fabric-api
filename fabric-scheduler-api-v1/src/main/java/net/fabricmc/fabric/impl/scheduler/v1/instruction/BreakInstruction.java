...
public final class BreakInstruction implements SchedulerInstructionImpl {
	@Nullable
	private final String label;

	public BreakInstruction(@Nullable String label) {
		this.label = label;
	}
	...
	public SchedulerNode compile(SchedulerNode successor, Map<String, SchedulerNode> nodeByStateId, Map<@Nullable String, List<UnboundExitNode>> unboundExitNodes) {
		...
		if (exits == null) {
			if (label != null) {
				throw new IllegalStateException("Break instruction references non-existent label '" + label + "'");
			}

			// no containing loop, treat the break like a return
