...
public final class LoopWhileInstruction implements SchedulerInstructionImpl {
	@Nullable
	private final String label;
	private final BooleanSupplier condition;
	private final SchedulerInstructionImpl body;

	public LoopWhileInstruction(@Nullable String label, BooleanSupplier condition, SchedulerInstructionImpl body) {
		this.label = label;
	...
	public SchedulerNode compile(SchedulerNode successor, Map<String, SchedulerNode> nodeByStateId, Map<@Nullable String, List<UnboundExitNode>> unboundExitNodes) {
		...
		ConditionNode conditionNode = new ConditionNode(condition, compiledBody, successor);
		loopbackNode.target = conditionNode;
		...
		for (UnboundExitNode exit : exits) {
			if (exit.instruction() instanceof BreakInstruction) {
				...
				exit.exitNode().target = conditionNode;
			}
		}
