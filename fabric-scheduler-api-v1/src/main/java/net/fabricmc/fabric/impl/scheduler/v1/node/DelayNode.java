...
public final class DelayNode implements SchedulerNode {
	private final String stateId;
	private final IntSupplier ticks;
	private final SchedulerNode next;

	public DelayNode(String stateId, IntSupplier ticks, SchedulerNode next) {
	...
	public Either<SchedulerNode, SchedulerStateImpl> execute(SchedulerStateImpl state) {
		...
		if (state.stateId().equals(stateId)) {
			...
			ticks = this.ticks.getAsInt();
		}

		if (ticks <= 0) {
			return Either.left(next);
		} else {
			return Either.right(new SchedulerStateImpl(stateId, ticks));
		}
	}
}
