...
public final class ServerLoginNetworking {
	...
	/// Use [#unregisterGlobalReceiver(Identifier)] to unregister the existing handler.
	///
	...
	/// Registers a handler to a query response channel.
	///
	/// If a handler is already registered to the `channelName`, this method will return `false`, and no change will be made.
	/// Use [#unregisterReceiver(ServerLoginPacketListenerImpl, Identifier)] to unregister the existing handler.
	///
	...
	///
	/// @param channelName the id of the channel
	/// @return the previous handler, or `null` if no handler was bound to the channel name
	...
	/// Returns the _Minecraft_ Server of a server login packet listener.
	///
	...
	public interface LoginQueryResponseHandler {
		...
		/// @param synchronizer the synchronizer which may be used to delay log-in till a [Future] is completed.
	...
	/// Allows blocking client log-in until all futures passed into [LoginSynchronizer#waitFor(Future)] are completed.
	@FunctionalInterface
	@ApiStatus.NonExtendable
	public interface LoginSynchronizer {
		/// Allows blocking client log-in until the `future` is [done][Future#isDone()].
		///
