...
public class RegistryDataLoaderResourceManagerRegistryLoadTaskMixin {
	...
	private <T> RegistryDataLoader.PendingRegistration<?> modify(ResourceKey<T> key, Either<T, Exception> value, RegistrationInfo registrationInfo, Operation<RegistryDataLoader.PendingRegistration<T>> original, @Local(argsOnly = true, name = "thunk") Resource thunk) {
		if (value.left().isPresent()) {
			if (value.left().get() instanceof Enchantment enchantment) {
				Enchantment modified = EnchantmentUtil.modify((ResourceKey<Enchantment>) key, enchantment, EnchantmentUtil.determineSource(thunk));
