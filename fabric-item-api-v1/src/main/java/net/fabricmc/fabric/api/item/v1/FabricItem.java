...
public interface FabricItem {
	...
	///
	/// @param player   the current player; this may be safely cast to [net.minecraft.client.player.LocalPlayer] in client-only code
	...
	///
	/// Note that simple item remainders can also be set via [Item.Properties#craftRemainder(Item)].
	///
	/// If you want to get a remainder for a stack,
	/// is recommended to use the stack version of this method: [FabricItemStack#getCraftingRemainder()].
	///
	...
	/// Determines if the item is allowed to receive an [Enchantment]. This can be used to manually override what
	/// enchantments a modded item is able to receive.
	///
	/// For example, one might want a modded item to be able to receive Unbreaking, but not Mending, which cannot be
	/// achieved with the vanilla tag system alone. Alternatively, one might want to do the same thing with enchantments
	/// from other mods, which don't have a similar tag system in general.
	///
	/// Note that this method is only called _after_ the [EnchantmentEvents#ALLOW_ENCHANTING] event, and
	/// only if none of the listeners to that event override the result.
	///
	...
	///
	/// This can be used if, for example, a library mod registers a generic item that other mods can create new
	/// variants for, allowing those mods to take credit for those variants if a player wishes to know what mod they
	/// come from.
	///
	/// Should be used instead of querying the item ID namespace to determine what mod an item is from when displaying
	/// to the player.
	///
	/// Defaults to the namespace of the item's own holder, except in the cases of potions or enchanted books,
	/// in which it uses the namespace of the potion contents or single enchantment applied.
	///
	/// Note that while it is recommended that this reflect a namespace and/or mod ID, it can technically be any
	/// arbitrary string.
	///
	/// @param stack the current stack
