...
/// tests, see the package documentation for details. To disable this, use [#setUseConsistentSettings]. If you need
/// to re-enable a particular setting, you can override it using [#adjustSettings].
...
public interface TestWorldBuilder {
	...
	///
	/// The dedicated server will only run if the EULA has been accepted in `eula.txt`. See
	/// [TestDedicatedServerContext] for details.
	///
	...
	/// [TestDedicatedServerContext] for details.
	///
	/// @param serverProperties The custom server properties to be written to the `server.properties` file.
	/// @return The dedicated server context of the server that was created.
