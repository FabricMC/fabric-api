...
/// the template image. See [TestScreenshotComparisonAlgorithm#defaultAlgorithm()] for details on how this is
/// implemented. To change the threshold for how different the screenshot can be, set the algorithm to
/// [TestScreenshotComparisonAlgorithm#meanSquaredDifference(float)]. To force the screenshot to be exactly the
/// same as your template, use [TestScreenshotComparisonAlgorithm#exact()]. It is recommended to stick with fuzzy
...
///
/// Template images loaded from a path are expected to be in the `templates` directory inside the resources
/// directory of the mod which registered the gametest. If the template image is not found then the screenshot will be
/// saved to the `templates` directory so that it can be used next time the gametest is run. Your test mod's
/// resources directory is in the `fabric.client.gametest.testModResourcesPath` system property which is set by
...
public interface TestScreenshotComparisonOptions extends TestScreenshotCommonOptions<TestScreenshotComparisonOptions> {
	/// Load the template image from a path. The template image is expected to be in the `templates` directory
