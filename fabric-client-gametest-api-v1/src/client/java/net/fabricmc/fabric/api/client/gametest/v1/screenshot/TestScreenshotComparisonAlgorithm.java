...
public interface TestScreenshotComparisonAlgorithm {
	...
	/// `0.005`. See [#meanSquaredDifference(float)] for details.
	///
	...
	/// @return The location of the match, or `null` if no match was found. In case of multiple matches, returns an
	///         arbitrary match
	@Nullable
	...
	///
	/// Note for custom algorithm implementations: the default implementation converts the images to color and then
	/// uses [#findColor], but it is usually possible to implement this more efficiently. You should strongly
	...
	interface RawImage<DATA> {
		...
		/// `y * this.width() + x`, where (`x`, `y`) are the coordinates of the pixel you want to
		/// index. The length of the array is `this.width() * this.height()`.
		///
