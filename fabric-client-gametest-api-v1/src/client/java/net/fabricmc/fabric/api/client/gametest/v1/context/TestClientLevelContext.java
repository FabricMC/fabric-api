...
public interface TestClientLevelContext {
	...
	/// downloaded after [#DEFAULT_CHUNK_LOAD_TIMEOUT] ticks. See [#waitForChunksDownload(int)] for details.
	///
	...
	/// [net.minecraft.client.multiplayer.ClientLevel#getChunk(int, int)] and [net.minecraft.client.multiplayer.ClientLevel#getBlockState(net.minecraft.core.BlockPos)] will return the expected
	/// value. However, the chunks may not yet be rendered and may not appear in screenshots, if you need this, use
	/// [#waitForChunksRender(int)] instead. Fails if the chunks haven't been downloaded after `timeout`
	/// ticks.
	///
	...
	/// [#DEFAULT_CHUNK_LOAD_TIMEOUT] ticks.
	///
	/// @return The number of ticks waited
	...
	/// Waits for all chunks to be downloaded and rendered. After this, all chunks that will ever be visible are visible
	/// in screenshots. Fails if the chunks haven't been downloaded and rendered after `timeout` ticks.
	///
	/// @param timeout The number of ticks before timing out
	/// @return The number of ticks waited
	...
	/// (and optionally downloaded) after [#DEFAULT_CHUNK_LOAD_TIMEOUT] ticks.
	///
	...
	/// Waits for all chunks to be rendered, optionally waiting for chunks to be downloaded first. After this, all chunks
	/// that are present in the client level will be visible in screenshots. Fails if the chunks haven't been rendered
	/// (and optionally downloaded) after `timeout` ticks.
	///
	/// @param waitForDownload Whether to wait for chunks to be downloaded
	/// @param timeout The number of ticks before timing out
	/// @return The number of ticks waited
