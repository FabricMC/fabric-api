...
/// <h2 id="sync">Synchronization
/// Dynamic registries are not synchronized to the client by default.
/// To register a _synced dynamic registry_, you can replace the [#register] call
...
public final class DynamicRegistries {
	...
	///
	/// The entries of the registry will be loaded from data packs at the file path
	/// `data/<entry namespace>/<registry namespace>/<registry path>/<entry path>.json`.
	///
	...
	///
	/// If the object contained in the registry is complex and contains a lot of data
	/// that is not relevant on the client, another codec for networking can be specified with
	/// [#registerSynced(ResourceKey, Codec, Codec, SyncOption...)].
	///
	/// @param key     the unique key of the registry
	...
	/// `data/<entry namespace>/<registry namespace>/<registry path>/<entry path>.json`
	///
	/// The registry will be synced from the server to players' clients using the given client codec.
	/// The client codec is also used for writing synced entries on the server, so it cannot depend on
	/// client-only code.
	///
	/// The client codec must be able to read data encoded by the server codec.
