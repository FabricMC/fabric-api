...
public final class TillableBlockRegistry {
	...
	///
	/// Tilling interactions are a two-step process. First, a usage predicate is run that decides whether to till
	/// a block. If the predicate returns `true`, an action is executed. Default instances of these can be created
	/// with these [HoeItem] methods:
	///
	///   - usage predicate for farmland-like behavior: [HoeItem#onlyIfAirAbove(UseOnContext)]
	///   - simple action: [HoeItem#changeIntoState(BlockState)] (BlockState)}
	///   - simple action that also drops an item: [HoeItem#changeIntoStateAndDropItem(BlockState, ItemLike)]
	///
	///
	/// @param input          the input block that can be tilled
	/// @param usagePredicate a predicate that filters if the block can be tilled
	/// @param tillingAction  an action that is executed if the predicate returns `true`
