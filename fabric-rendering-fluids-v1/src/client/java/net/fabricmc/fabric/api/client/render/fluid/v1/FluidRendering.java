...
public final class FluidRendering {
	...
	/// [FluidRenderHandler#renderFluid], but the passed default renderer is invoked instead of the vanilla
	/// renderer whenever the handler requests default geometry to be rendered.
	///
	/// @param handler the render handler to invoke [FluidRenderHandler#renderFluid] on
	...
	public interface DefaultRenderer {
		...
		///
		/// @param handler the handler that [FluidRenderHandler#renderFluid] was invoked on
		/// @param level the level
		/// @param pos the pos
		/// @param vertexConsumer the vertex consumer
		/// @param blockState the block state
