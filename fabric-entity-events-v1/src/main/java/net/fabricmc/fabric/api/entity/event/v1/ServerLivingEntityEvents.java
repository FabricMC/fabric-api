...
public final class ServerLivingEntityEvents {
	...
	/// This is fired from [LivingEntity#hurtServer], before armor or any other mitigation are applied.
	...
	/// An event that is called after an entity is damaged. This is fired from [LivingEntity#hurtServer] after damage
	...
	///
	/// Vanilla checks for entity health `<= 0` each tick (with [LivingEntity#isDeadOrDying()]), and kills if true -
	...
	///
	/// When this event is called, the old instance has not yet been discarded, and the new one has not yet been spawned.
	/// Mods may use this event to copy some of the old entity's data to the converted one.
	///
	/// This event only handles cases where the entity type changes, requiring a new instance. Notably it does not
	/// cover mooshrooms changing color from lightning, creepers getting charged, or wolves being tamed.
